...
class lcurve:
    ...
    def lc(self, time, expose=0, ndiv=1,
       #Binary and stars
       ...
       t1_value = 15918.02,  t1_range = 0.,  t1_dstep = 0.,  t1_vary = False,  t1_defined = False,
       t2_value =  34570.39,  t2_range = 0.,  t2_dstep = 0.,  t2_vary = False,  t2_defined = False,
       ...
       info = False
        ...
        '''
        Np = len(time)
        if isinstance(expose, float): expose = expose*np.ones(Np, dtype=np.double)
        if isinstance(ndiv, float): ndiv = ndiv*np.ones(Np, dtype=np.int)
        libpylcurve=ctypes.CDLL(f"{libdir}/libpylcurve.so") 
        print('ok')
